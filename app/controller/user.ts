...
export default class UserSearch extends Controller {
    ...
    async index() { // GET - /XXX - XXX
        ...
        let hasCustomer: boolean = await service.user.hasCustomer(userName); // service/user.ts/isCustomer()
        let body: LoginRes;

        if(hasCustomer) {
            let isCustomer: boolean = await service.user.isCustomer(userName, passWord);
            if(isCustomer) {
                let {sex, userID, avatar, msgNum, registerDate, lastOnlineDate, lastOnlineMsg} = await service.user.findUserInfo(userName);
